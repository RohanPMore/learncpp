...
void findInterfaceAndPin(unsigned int hexInput) {
    ...
    // Apply 64-bit masking to isolate the lowest-order `1` bit
    unsigned int maskedInput = hexInput & (-hexInput);

    // Convert the masked input to binary (64 bits)
    bitset<64> binary(maskedInput);
